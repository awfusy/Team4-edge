...
class OptimizedCentralHub:
    def __init__(self, broker_address='192.168.211.254', broker_port=1883, reconnect_delay=2, publish_retry_delay=1):
        ...
        self.message_queue = queue.Queue(maxsize=max(100, int(available_memory_mb / 2)))  # At least 100
        self.proximity_publish_queue = queue.Queue(maxsize=max(50, int(available_memory_mb / 4)))
        self.qos2_publish_queue = queue.Queue(maxsize=max(100, int(available_memory_mb / 2)))
    ...
    def reconnect_with_fixed_delay(self):
        ...
        aggressive_delay = 1   # 0.5 seconds for aggressive phase
